...
I would like your assistance in writing concise, modular notes on the topic of [Support Vector Machines] The goal is to create notes that are easy to understand and focused on key ideas rather than verbatim copying from textbooks. 
...
For now, please ONLY write a motivation callout that introduces a problem (model that is really good at lots of things, but with smaller datasets but bad with bigger ones)

no other callouts at this time. 
