...
> [!consider] How Regularization Reduces Overfitting
> 
> Regularization directly combats overfitting by constraining the model's ability to fit noise in training data.
> 
> **Mechanism of Action**
> 
> - **Reduces weight magnitudes**: Large weights allow models to create sharp, complex decision boundaries that fit noise rather than signal. By shrinking weights, regularization smooths these boundaries.
> - **Parameter space restriction**: Regularization creates a smaller, constrained space of possible parameter values the model can explore during training.
> - **Statistical view**: Acts as a prior belief that smaller parameter values are more likely to generalize well.
> 
> **Effect on Learning**
> 
> - Forces the model to distribute importance across more features rather than relying heavily on a few
> - Prevents the model from fitting random fluctuations by making such fits "expensive" in terms of the penalty
> - Creates simpler functions that vary more smoothly as inputs change
> 
> In neural networks, L2 regularization reduces the curvature of the learned function, resulting in more gradual transitions between predictions. This makes the model less sensitive to small changes in input—a key characteristic of models that generalize well to unseen data.