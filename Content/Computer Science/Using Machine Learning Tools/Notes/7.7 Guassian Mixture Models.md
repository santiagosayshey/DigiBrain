...
> [!idea] Bayesian GMM: Automatic Cluster Number Selection
>
> Selecting the right number of clusters ($k$) for standard Gaussian Mixture Models (GMMs) can be challenging, often requiring testing multiple $k$ values and using criteria like BIC or AIC. **Bayesian Gaussian Mixture Models (BGMMs)** provide an alternative method to address this.
>
> **Understanding Component Weights ($\phi$) in GMMs:**
> Before seeing how BGMM works, recall that a standard GMM estimates parameters for each of its $k$ Gaussian components (ellipsoids). One of these parameters is the **weight** ($\phi$).
> * The weight $\phi_j$ for component $j$ represents the **proportion of the data** believed to be generated by that component, or its **relative importance/size** within the overall mixture.
> * All component weights ($\phi_1, \phi_2, ..., \phi_k$) sum up to 1.
>
> **How Bayesian GMM Automates *k* Selection:**
> The `BayesianGaussianMixture` class uses a Bayesian approach that allows it to automatically adjust these component **weights** ($\phi$) during the fitting process.
> * **Mechanism**: If a component is found to be unnecessary for explaining the data, the model can effectively "prune" it by assigning it a **weight ($\phi$) very close to zero**.
> * **Usage**: You initialize the `BayesianGaussianMixture` with a number of components (`n_components`) that is likely *larger* than the true optimal number. The algorithm then determines which of these initial components are actually needed.
> * **Result**: After fitting (`bgm.fit(X)`), you inspect the learned `weights_` attribute. Components corresponding to near-zero weights are the ones the model deemed redundant. The number of components with significant (non-zero) weights indicates the effective number of clusters the model found necessary. For instance, setting `n_components=10` might result in only 3 components having significant weights, indicating the model selected $k=3$.
>
> This allows the BGMM approach to infer the number of active clusters from the data, rather than requiring manual selection via separate criteria.
>
> **Important Limitation:** Remember that BGMMs **still assume the underlying clusters are ellipsoidal**. They will struggle to correctly identify clusters with fundamentally non-ellipsoidal shapes (like the moons dataset).
>
> ```image_goes_here
