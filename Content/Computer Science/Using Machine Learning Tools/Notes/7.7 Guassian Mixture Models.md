...
> [!idea] Bayesian GMM: Automatic Cluster Number Selection
>
> A significant challenge with standard Gaussian Mixture Models (GMMs) is selecting the optimal number of clusters ($k$). Often, this requires fitting multiple models and comparing criteria like BIC or AIC. **Bayesian Gaussian Mixture Models (BGMMs)** offer an alternative approach to address this.
>
> **Understanding Cluster Weights ($\phi$) in GMMs:**
> Recall that GMMs estimate parameters for each of the $k$ **Gaussian clusters** (or components) they model. One parameter is the **weight** ($\phi$) for each cluster. This weight represents the **proportion of the data** expected to belong to that cluster, or its **relative importance/size** in the overall mixture. These weights sum to 1.
>
> **How Bayesian GMM Automates *k* Selection:**
> The `BayesianGaussianMixture` class uses a method that automatically adjusts these cluster **weights** ($\phi$) during fitting.
> * **Mechanism**: If a potential cluster is found to be unnecessary to explain the data, the model effectively "prunes" it by assigning its corresponding **weight ($\phi$) a value very close to zero**.
> * **Usage**: You initialize the model with `n_components` (the initial number of **clusters** to consider) set to a value likely *larger* than the true number. The algorithm then determines which are actually needed.
> * **Result**: After fitting (`bgm.fit(X)`), inspecting the `weights_` attribute shows which **clusters** have significant (non-zero) weights. Those with near-zero weights were deemed redundant, revealing the effective number of clusters the model selected. For instance, setting `n_components=10` might result in only 3 clusters having significant weights, indicating the model determined $k=3$ was optimal.
>
> This allows the BGMM approach to infer the number of active clusters from the data, avoiding manual search.
>
> **Important Limitation:** BGMMs **still assume the underlying clusters are ellipsoidal**. They perform poorly on data with non-ellipsoidal shapes (like the moons dataset), potentially fitting multiple ellipsoids incorrectly.
>
> ```image_goes_here
> Caption: Figure 9-21. Bayesian GMM applied to non-ellipsoidal clusters.
> Description: A plot showing the moons dataset. Superimposed are the ellipsoids found by a Bayesian GMM (e.g., initialized with k=10). Instead
