...
I would like your assistance in writing concise, modular notes on the topic of [Linear Regression] The goal is to create notes that are easy to understand and focused on key ideas rather than verbatim copying from textbooks. 
...
motivation - its typically enough to know that linear regression is a model for ml, but it helps to understand it fully because its used as a basis for so many other things

idea - linear regression
- go in depth here, maths and all. i expect to see inline latex. 

example

no other callouts for now please.
