...
>
> The Perceptron, invented by Frank Rosenblatt in 1957, introduced a more sophisticated artificial neuron called the Threshold Logic Unit (TLU) to replace the simple McCulloch-Pitts model.
>
> TLUs enhance the basic neuron model in several important ways:
> - They process numerical inputs rather than binary on/off values
> - Each connection has a weight that reflects its importance
> - A bias term is added to adjust the activation threshold
>
> A TLU processes information in two straightforward steps:
>
> First, it computes a weighted sum of all inputs plus a bias term:
> $z = w_1x_1 + w_2x_2 + ... + w_nx_n + b = \mathbf{w}^T\mathbf{x} + b$
>
> Then it applies a step function that produces a binary output:
> ```
>              w₁
>   x₁ ─────────→ \
>              w₂   \
...
> ```
>
> The step function creates a sharp decision boundary:
> - If the weighted sum is ≥ 0, the output is 1
> - If the weighted sum is < 0, the output is 0
>
> A complete Perceptron connects inputs to multiple TLUs in a single layer, with each TLU working independently to detect different patterns. This structure enables the network to perform classification tasks like recognizing simple patterns or separating data into categories.
>
> While revolutionary for its time, the Perceptron has a fundamental limitation discovered by Minsky and Papert: it can only solve linearly separable problems (those where categories can be separated by a straight line). This constraint eventually led to the development of multi-layer networks.