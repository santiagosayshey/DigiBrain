...
> [!example] McCulloch-Pitts Neuron Model (1943)
>
> The McCulloch-Pitts neuron represents the first mathematical model of a biological neuron as a computational unit. It functions as a binary threshold device:
>
...
>
> ```image-goes-here
> Description: Diagram of a McCulloch-Pitts neuron showing:
> 
> 1. Three excitatory inputs labeled x₁, x₂, and x₃ (with values 1, 0, 1) entering from the left side, each with an arrow pointing toward the neuron body.
> 
> 2. Two inhibitory inputs labeled x₄ and x₅ (with values 0, 0) entering from the left, each with a circle at the end of the connection line (representing inhibition) before entering the neuron body.
> 
> 3. The neuron cell body represented as a large circle in the center, containing "Σ ≥ θ" to show the summation and threshold comparison.
> 
> 4. The threshold value θ = 2 shown near the neuron body.
> 
> 5. A single output line exiting from the right side of the neuron, labeled with "y = 1" to show the binary output.
> 
> 6. Clear labels showing the calculation: "Sum of excitatory inputs = 1 + 0 + 1 = 2", and "No active inhibitory inputs" and "2 ≥ 2, therefore output = 1".
> 
> 7. Optional caption stating: "McCulloch-Pitts neuron (1943): The first computational model of a biological neuron, functioning as a logical threshold unit."
