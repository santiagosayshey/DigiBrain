...
- **Topic for Notes:** `[Neural Networks]`
...
| --------- | ---------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1         | Motivation       | Learning From Nature  | Here we want to talk about how nature influences our inventions - birds flying, burdock plants velcro, human brain into intelligent machines.<br><br>Then we should go into the problem of large, complex machine learning tasks, with a brief introduction to deep learning. We dont want to explicitly state what Deep Learning is, just frame it as an issue.<br><br>This then of course leads into the actual motivation for ANNs, then list some examples - siri, google images, youtube recommendations, etc. Remember, we are not explicitly stating ANN yet, just exploring its motivation.<br><br>i also think we should briefly explain how humans process information and why its so good.<br><br>so the big ideas we want to introduce are - 1. humans invent things by drawing inspiration from nature 2. huge problem to build machines that can do extremeply complex learning on its own 3. hey wait, dont OUR brains already do that!!! |
...
> [!idea] The Perceptron and Threshold Logic Units
>
> The Perceptron, invented by Frank Rosenblatt in 1957, marked a revolutionary step beyond the basic McCulloch-Pitts neuron by introducing the ability to learn from examples.
>
> At its core is the Threshold Logic Unit (TLU), which improves upon earlier models in several key ways:
> - It processes numerical inputs instead of just binary signals
> - Each connection carries a weight that can be adjusted through learning
> - It includes a bias term that helps position the decision boundary
>
> A TLU transforms inputs into a decision through two simple steps:
>
> First, it computes a weighted sum of all inputs plus a bias term:
> $z = w_1x_1 + w_2x_2 + ... + w_nx_n + b = \mathbf{w}^T\mathbf{x} + b$
>
> Then it applies a step function to make a binary decision:
> ```
>              w₁
>   x₁ ─────────→ \
>              w₂   \
>   x₂ ─────────→    \    ┌───────┐
>              w₃     ├───┤Σw⊺x+b │────→ step(z) ────→ Output (0 or 1)
>   x₃ ─────────→    /    └───────┘
>              wₙ   /        ↑
>   xₙ ─────────→ /          │
>                             b (bias)
> ```
>
> This step function creates a sharp boundary - like a light switch that's either on or off. For the Heaviside step function:
> - If the weighted sum is positive or zero: output is 1
> - If the weighted sum is negative: output is 0
>
> What made the Perceptron truly groundbreaking was its learning algorithm. The weights start randomly and are adjusted whenever the model makes a mistake:
>
> If the TLU incorrectly remains silent when it should activate, weights for active inputs are increased. If it incorrectly activates when it should stay silent, those weights are decreased. This elegantly simple approach will eventually converge on a solution for any problem where the categories can be separated by a straight line (or hyperplane in higher dimensions).
>
> A complete Perceptron can have multiple TLUs forming an output layer, with each unit potentially detecting different patterns or categories in the input data.
>
> Despite its innovations, the Perceptron had a critical limitation discovered by Minsky and Papert in 1969: it cannot learn patterns that aren't linearly separable, such as the XOR problem (where outputs are 1 only when inputs are different). This limitation eventually led researchers to develop multi-layer networks that could overcome this constraint.