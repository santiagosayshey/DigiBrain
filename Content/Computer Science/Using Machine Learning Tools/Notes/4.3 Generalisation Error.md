...
I would like your assistance in writing concise, modular notes on the topic of [Generalisation Error] The goal is to create notes that are easy to understand and focused on key ideas rather than verbatim copying from textbooks. 
...
motivation - we want to find generalisation error to improve our models, so it would help to find different sources that they come from

idea - variance. irreducible error. overfitting
idea - bias/systematic. sub optimal model/hyperparameter choices, representativeness of data
