...
| B413         | High         | High           | client.py:15 | The `pyCrypto` library and its module `get_random_bytes` are deprecated and no longer maintained. Using deprecated libraries can introduce security vulnerabilities.                                   | Use `os.urandom()` or functions from `pyca/cryptography` for generating random bytes securely.                                                                                          | [Link](https://bandit.readthedocs.io/en/latest/blacklists/blacklist_imports.html#b413-import-pycrypto)     |
...
Due to the lack of documentation and absence of a README file, setting up the testing environment was challenging. The codebase contains multiple files, some of which are not relevant to the current implementation. The hardcoded addresses and ports limit the ability to test the application in a multi-server or multi-client setup. After inspecting the code, the reviewer attempted to run the `server.py` and `client.py` files to evaluate the application's functionality.
