...
> Consider fuzzing system calls in a Linux kernel:
> 1. Create description of each system call's format:
>    - open() needs a file path and permissions
>    - read() needs a file handle and buffer size
>    - mmap() needs memory size and protection flags
> 2. Generate test sequences:
>    - Create valid system call chains (open file, read it, close it)
>    - Insert unusual arguments (negative sizes, invalid flags)
>    - Maintain correct order (can't read before open)
> 3. Run in isolated VM and watch for kernel panics
> 
> This is how Syzkaller works - it understands system call formats and generates millions of valid but unusual combinations to find kernel bugs.

> [!example] Coverage-Guided Fuzzing in Action
> Consider fuzzing a JPEG image parser:
> 1. Start with a normal JPEG file
> 2. Parser shows it processes these sections in order:
>    - Header markers
>    - Color space info
>    - Huffman tables
>    - Image data
> 3. When fuzzer changes header markers:
>    - Notices new error handling code executes
>    - Keeps mutations that hit new code paths
>    - Focuses on areas that trigger new behaviors
> 4. Eventually finds bug in rarely-used compression options
> 
> This is how AFL works - by watching which code paths each test case hits, it learns which mutations are most likely to find bugs.

Is this better? Each example now explains the technique through a concrete, practical scenario that shows exactly how that type of fuzzing works?
