...
motivation - up to you to decide what should set the stage for a motivation here. Dont make it for reviewing code in genral, but why static analysis is needed

idea - static analysis
- systematic (what does this actually mean) examination of an abstraction of program state space (completely reword this to sound less confusing)
- can produce false positives as it uses an abstraction, not result
- white box
- multiple methods
	- pattern matching
		- lexical analysis to find potentially dangerous patterns / bad practices
	- data flow analysis
		- information about the possible values propagating across the program - validate safety properties around sensitive operations
	- symbolic execution
		- used when concreete vqalues are unknown - then used to perform data flow analysis
