...
I would like your assistance in writing concise, modular notes on the topic of [Euthanasia] The goal is to create notes that are easy to understand and focused on key ideas rather than verbatim copying from textbooks. 
...
motivation - a good death. we want to talk about situations in which the death of a person could be considered a desired outcome. 

consider - euthenasia versus physician assisted suicide
- eutenasia is when a docter ends a patients life (either by killing them or letting them die)
- PAS is when the patient takes their own life with the aid of a physician
- an example for each (not a seperate callout, inside this consider one)
- also maybe a note on whether PAS is a form of 'letting die'

idea - euthenasia. just start with a general overview then move into a clear distinction of active and passive euthenasia. also note that while there are many countries where euthenasia is permitted (even active, list some), there are many countries where it is entirely illegal. then list the 3 kinds of euthenasia
- voluntary - performed with a parients voluntary consent (also note that what this actually is, is quite controversal but for the sake of understanding, its the patient has been gfiven enough information, understands it, and makes a decision absent of coercion)
- non voluntary - performed on a patient who can't make a voluntary choice and has not said so before

example - a well known case study of euthenasia, preferably with some controversial undertones. maybe we could even go over that one SVU episode for that baby euthenasia? the doctor helps the mother kill her baby that will die? let me know if you have enough info about htis to go forward with it, otherwise just leave it as blank and i can give you a synopsis / more info about the episode. 
