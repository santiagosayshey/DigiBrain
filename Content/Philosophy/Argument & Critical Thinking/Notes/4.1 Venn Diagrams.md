...
I would like your assistance in writing concise, modular notes on the topic of [Venn Diagrams] The goal is to create notes that are easy to understand and focused on key ideas rather than verbatim copying from textbooks. 
...
motivation - issues with analgous form and counterexamples to disprove arguments

idea - venn diagrams. representation of classes, categorical statements and arguiments built from such statements

idea - classes. groups of things. things are called members. 

example - recall subject and predicate terms in categorical statements and how these are classes. with examples. 

consider - classes dont necessarily need to contain things that actually exist. they just need to convey things that we can imagine exisating. like superheroes or unicorns

example - representing a class (please generate an svg here) and describe its contents in the callouit. do the svg in asepearte artrifact once youre done. start with the universe resprented by a rectangle. it contains everything. then we have a circle inside the universe for cats. another for gods. yes gods. not dogs. have them overlap. label everything
