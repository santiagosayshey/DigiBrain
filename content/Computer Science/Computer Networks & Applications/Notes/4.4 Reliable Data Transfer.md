...
> ![[aaa.png]]
> 
> We should also let the sender know which message we are replying too by adding the sequence number to the ACK. We can remove NAKs by adding the previous sequence number to ACKs to let the sender know that it was corrupt or had the wrong sequence number.
