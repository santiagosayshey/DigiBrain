...
I would like your assistance in writing concise, modular notes on the topic of [Software Defined Networking] The goal is to create notes that are easy to understand and focused on key ideas rather than verbatim copying from textbooks. 
...
The motivation of this callout shoud explain that traditional control and data plane based routing is rigid and makes it hard to change optimal routes based on anything other than the shortest path. Go into detail on the idea of SDNs and how they fix this. Add an example for defining a route for sensitive data th
