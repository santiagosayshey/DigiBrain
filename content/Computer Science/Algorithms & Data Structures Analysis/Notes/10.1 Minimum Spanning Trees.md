...
> [!idea] Cycle Property
>
> The Cycle Property states that an **edge of maximum cost/weight in any cycle does not need to be considered** for computing a Minimum Spanning Tree (MST).
>
> The reasoning behind this property is that if a cycle contains an edge of maximum cost/weight, removing that edge would result in a subgraph with a lower total cost/weight. Since an MST does not contain any cycles, the maximum-cost edge in a cycle can be safely excluded from consideration.
>
> The Cycle Property is useful for pruning unnecessary edges during the construction of an MST, potentially improving the efficiency of the algorithms.
