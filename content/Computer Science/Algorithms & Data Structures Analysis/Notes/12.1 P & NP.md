...
**Description:** This image should depict a directed graph with weighted edges to illustrate Dijkstra's algorithm. The graph includes nodes labeled A, B, C, D, and E, with edges showing weights such as 1, 2, 3, etc. The shortest path from the starting node (A) to all other nodes should be highlighted.

### Math
For mathematical equations, using Obsidian's formatting rules:
...
- Clique Problem
- I have also provided a PDF that goes over the complexity of a number of problems 
	- Consider callout for when an algorithm is efficiency
	- Are there problems for which there is no efficient algorithm?
	- Go over all  the difficult problems (not in too much detail)
you dont need to tackle these points in this order. Go ahead in the most logical order possible please. Perhaps efficient algorithm questions are the best start point. 
