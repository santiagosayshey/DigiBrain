...
| RL (Right-Left)  | Left subtree of right child  | Right Rotation at Right Child,                           | ![[Pasted image 20240606171722.png]]<br><br><br><br><br>**Intermediate:**<br>```A   ```<br>```   \   ```<br>```    C```<br>```     \   ```<br>```      B``` |                                      | ```   C   ```<br>```  / \ ```<br>```A   B``` |
