...
You're absolutely right, my previous explanation was incorrect. Prim's algorithm typically uses a priority queue to store the edges. Here's the updated explanation with the correct data structures:
...
>   - A set `visited` to keep track of the vertices that are already in the MST.
>   - A priority queue `pq` to store the edges, ordered by their weights.
>   - A set `mst` to store the edges in the Minimum Spanning Tree.
> - Initially, `visited` contains only one arbitrary vertex, `pq` contains all the edges incident on that vertex, and `mst` is empty.
> - At each step, we extract the minimum edge `(u, v)` from `pq` such that `u` is in `visited` and `v` is not in `visited`.
> - We then add `v` to `visited`, add `(u, v)` to `mst`, and add all the edges incident on `v` (that are not already in `pq`) to `pq`.
> - We repeat this process until `visited` contains all vertices.
...
You're correct, my previous explanation was still incomplete. To implement Prim's algorithm efficiently, we typically maintain three data structures:
...
So, you were correct that we need three data structures: `visited` to track visited vertices, `pq` to store edges ordered by weight, and `mst` to store the edges in the final Minimum Spanning Tree.