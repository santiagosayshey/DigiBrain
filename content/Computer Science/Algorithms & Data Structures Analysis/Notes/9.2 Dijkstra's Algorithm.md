...
function DijkstraShortestPath(Graph, startNode):
   ...
   priorityQueue.enqueue(startNode, startNode.distance)   # O(log n) for BH, O(1) for Fibonacci heap

   while priorityQueue not empty:                         # O((n + m) log n) for binary heap, O(m + n log n) for Fibonacci heap
       currentNode = priorityQueue.dequeueMin()           # O(log n) for binary heap, O(log n) amortized for Fibonacci heap
       ...
       for each neighbor of currentNode:                  # O(degree(currentNode))
           ...
           if distanceThroughCurrent < neighbor.distance:
               ...
               priorityQueue.enqueue(neighbor, neighbor.distance)   # O(log n) for binary heap, O(1) for Fibonacci heap

   return distances, previousNodes
```

In summary, the time complexity of Dijkstra's algorithm using a binary heap is $O((m + n) \log n)$, while using a Fibonacci heap yields a complexity of $O(m + n \log n)$. The choice between the two depends on the graph's characteristics and the specific implementation details.