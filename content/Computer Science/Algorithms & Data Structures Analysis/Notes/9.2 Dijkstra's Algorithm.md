...
>
> Dijkstra's algorithm finds the shortest paths from a starting node to all other nodes in a weighted graph. The time complexity depends on the priority queue implementation used.
>
> - $n$: number of nodes
> - $m$: number of edges
>
...
>        - If the new distance is smaller than the current distance, update the distance and insert or update the neighbor in the priority queue.
>
> The time complexity of step 2 depends on the priority queue implementation:
>
> - Binary Heap:
>   - Extracting the minimum node takes $O(\log n)$ time.
>   - Inserting or updating a node takes $O(\log n)$ time.
>   - The total number of extract-min and insert/update operations is proportional to the number of nodes and edges.
>   - Total time complexity: $O((n + m) \log n)$
>
> - Fibonacci Heap:
>   - Extracting the minimum node takes $O(\log n)$ amortized time.
>   - Inserting a node takes $O(1)$ amortized time.
>   - Updating a node (decrease-key operation) takes $O(1)$ amortized time.
>   - The total number of extract-min operations is proportional to the number of nodes, and the total number of insert and decrease-key operations is proportional to the number of edges.
>   - Total time complexity: $O(m + n \log n)$
>
> The $m \log n$ term in the time complexity comes from the fact that the algorithm may need to perform priority queue operations (insertions or updates) for each edge in the graph.
>
