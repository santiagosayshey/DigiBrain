...
> [!idea] Processes
> A process is an execution stream with an associated process state, representing a program in active execution. Multiple independent instances of the same program can run as distinct processes. Each process operates under the illusion that it has exclusive use of the CPU, unaware of other processes competing for the same resources.
> 
